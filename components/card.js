  ...
  return (
    <Link href="/card/[token.identifier]" as={`/card/${token.identifier}`}>
      <div
        ...
        className="shadow-lg w-60 border max-w-sm mx-auto rounded-xl  p-2 bg-white  relative overflow-hidden"
      >
        ...
        <div className="mt-4">
          <p>Current Supply: {token.totalSupply.valueExact}</p>
          <p>Transfers: {data.transfers}</p>
          <p>Minted: {data.mints}</p>
          <p>Burned: {data.burns}</p>
        </div>
      </div>
    </Link>
  );
